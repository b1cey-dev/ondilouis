...
export default function TeamMembersClient({ params }: TeamMembersClientProps) {
  ...
  useEffect(() => {
    ...
    fetchTeamData()
  }, [session, router, params.teamId])

  const fetchTeamData = async () => {
    try {
      // Fetch team details
      const teamResponse = await fetch(`/api/admin/teams/${params.teamId}`)
      ...
      const membersResponse = await fetch(
        `/api/admin/teams/${params.teamId}/members`
      )
  ...
  const addTeamMember = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/admin/teams/${params.teamId}/members`, {
        method: "POST",
  ...
  const removeTeamMember = async (userId: string) => {
    try {
      const response = await fetch(
        `/api/admin/teams/${params.teamId}/members/${userId}`,
        {
          method: "DELETE",
        }
      )
